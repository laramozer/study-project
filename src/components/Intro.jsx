  ...
  return (
    <section className="bg-black py-20 px-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-center items-center mb-16">
        <button className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-full text-lg font-medium transition-all duration-300 hover:scale-105 shadow-lg shadow-blue-500/50">
          Compre agora
        </button>
        <button className="cursor-pointer border-2 rounded-full hover:bg-white hover:text-black text-white px-8 py-4 text-lg font-medium transition-all duration-300 hover:scale-105">
          Saiba mais
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mx-auto max-w-5xl">
        {infos.map((info, index) => (
          ...
          >
            ...
            <p className="text-sm text-gray-300 mt-2">{info.label}</p>
          </div>
        ))}
      </div>
    </section>
